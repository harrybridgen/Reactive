...
impl VM {
    ...
    pub(crate) fn as_int(&mut self, v: Type) -> i32 {
        match self.force(v) {
            ...
            Type::ArrayRef(id) => self.array_heap[id].len() as i32, // coerce arr into len
            Type::VecRef(id) => self.vec_heap[id].len() as i32,     // coerce arr into len
            other => self.runtime_error(&format!("type error: cannot coerce {:?} to int", other)),
        }
    }

    pub(crate) fn as_usize(&mut self, v: Type, what: &str) -> usize {
        let i = self.as_int(v);
        if i < 0 {
